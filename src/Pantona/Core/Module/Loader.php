...
use Pantona\Core\Exception\Bootstrap\Listener;
use Pantona\Core\Exception\Bootstrap\Routes;
        ...
        foreach ($this->getModuleConfig() as $config) {
            if (!empty($config['services'])) {
                foreach ($config['services'] as $name => $service) {
                    $this->application->addPantonaService($name, $service['class'], $service['arguments']);
                }
            }
        }
        return true;
